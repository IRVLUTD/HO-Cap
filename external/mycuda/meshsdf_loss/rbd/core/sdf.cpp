...
float* MakeSDF(const Mesh* mesh, Vec3 lower, Vec3 upper, uint32_t resolution, bool solid, int& dimx, int& dimy, int& dimz)
{
	...
	double startTreeBuild = GetSeconds();

	AABBTree tree((const Vec3*)mesh->m_positions.data(), uint32_t(mesh->m_positions.size()), mesh->m_indices.data(), uint32_t(mesh->m_indices.size())/3);

	double endTreeBuild = GetSeconds();

	Vec3 edges = (upper-lower);
	const int longestEdge = LongestAxis(edges);
	
	const float cellSize = edges[longestEdge]/resolution;

	dimx = int(ceilf(edges[0]/cellSize));
	dimy = int(ceilf(edges[1]/cellSize));
	dimz = int(ceilf(edges[2]/cellSize));

	float* output = new float[dimx*dimy*dimz];

	double startGridBuild = GetSeconds();
...
#pragma omp parallel for
			...
			{
				// voxel center
				Vec3 p = lower + Vec3(cellSize)*Vec3(x + 0.5f, y + 0.5f, z + 0.5f);

				float dist, u, v, w;
				uint32_t face;

				tree.GetClosestPointWithinDistance(p, FLT_MAX, dist, v, w, face);

				// test if point is inside the shape
				if (solid)
				{
					float signs[3] = { 0.0f };
					float t;

					tree.TraceRay(p, Vec3(1.0f, 0.0f, 0.0f), t, u, v, w, signs[0], face);
					tree.TraceRay(p, Vec3(0.0f, 1.0f, 0.0f), t, u, v, w, signs[1], face);
					tree.TraceRay(p, Vec3(0.0f, 0.0f, 1.0f), t, u, v, w, signs[2], face);

					if (signs[0] < 0.0f || signs[1] < 0.0f || signs[2] < 0.0f)
					{
						dist *= -1.0f;
					}
				}

				output[z*dimx*dimy + y*dimx + x] = dist;
			}
		}
	}

	double endGridBuild = GetSeconds();;
	
	printf("SDF: Tree build %f\n", (endTreeBuild-startTreeBuild));
	printf("SDF: Grid build %f\n", (endGridBuild-startGridBuild));
