...
CUDA_CALLABLE inline float DistanceToAABB(const Vec3& p, const Vec3& lower, const Vec3& upper)
{
...
CUDA_CALLABLE inline void TransformBounds(const Vec3& localLower, const Vec3& localUpper, const Vec3& translation, const Quat& rotation, const Vec3 scale, Vec3& lower, Vec3& upper)
{
	Matrix33 transform(rotation);

	Vec3 extents = (localUpper - localLower);
	extents.x *= scale.x;
	extents.y *= scale.y;
	extents.z *= scale.z;
...
{
	TransformBounds(localLower, localUpper, translation, rotation, Vec3(scale, scale, scale), lower, upper);
}
...
/*
bool BuildEdges(const int* triIndices, int numTriangles, std::vector<int>& edges, std::vector<int>& edgeAdj)
{
	struct Edge
	{
		Edge(int i, int j, int face)
		{
			vertices[0] = Min(i, j);
			vertices[1] = Max(i, j);
		...
		inline bool operator < (const Edge& e) const
		{
			...
			if (vertices[0] < e.vertices[0])
				return true;
			else if (e.vertices[0] < vertices[0])
				return false;

			if (vertices[1] < e.vertices[1])
				return true;
			else if (e.vertices[1] < vertices[1])
				return false;
	...
	for (int tri=0; tri < numTriangles; ++tri)
	{
		for (int v=0; v < 3; ++v)
		{
			Edge e(triIndices[tri*3 + v], triIndices[tri*3 + (v+1)%3], tri);

			EdgeSet::iterator iter = edgeSet.find(e);
			if (iter != edgeSet.end())
			{
				if (iter->faces[1] != -1)
					manifold = false;
	...
	{
		edges.push_back(edge.vertices[0]);
		edges.push_back(edge.vertices[1]);

		edgeAdj.push_back(edge.faces[0]);
		edgeAdj.push_back(edge.faces[1]);
	}
