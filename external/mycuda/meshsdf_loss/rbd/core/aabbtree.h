...
public:
	...
	void TraceBounds(const Vec3& lower, const Vector3& upper, std::vector<int>& triangles) const;

	bool GetClosestPointWithinDistance(const Vec3& point, const float maxDis, float& dis, float& v, float& w, uint32_t& faceIndex) const;
	bool AnyPointWithinDistance(const Vec3& point, const float maxDis) const;
    void DebugDraw();
...
private:
	...
	void TraceBoundsRecursive(uint32_t nodeIndex, const Vec3& lower, const Vector3& upper, std::vector<int>& triangles) const;

	void GetClosestPointWithinDistanceSqRecursive(uint32_t nodeIndex, const Vec3& point, float& outDisSq, float& outV, float& outW, uint32_t& outFaceIndex) const;
	bool AnyPointWithinDistanceSqRecursive(uint32_t nodeIndex, const Vec3& point, const float maxDisSq) const;
